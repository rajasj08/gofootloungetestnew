...
class ModelCatalogSearchAnalytics extends Model {
	...
	public function create_tables() {
		...
		$this->db->query("CREATE TABLE IF NOT EXISTS " . DB_PREFIX . "search_history(id int(255) NOT NULL AUTO_INCREMENT, keyphrase varchar(255), customer_id int(255), ip varchar(255), timestamp TIMESTAMP, PRIMARY KEY(id)) ENGINE = MyISAM");
	}
			...
			if ( isset($filter_data['match_type']) && $filter_data['match_type'] == 'broad' ) {
				...
				if (!empty($keyphrases)) {
					...
					//	$keyphrase_statements[] = " sh.keyphrase = '" . $keyword . "' ";	
						$keyphrase_statements[] = " sh.keyphrase REGEXP '[[:<:]]" . $keyword . "[[:>:]]' ";
					}
				}
				...
				foreach ($keyphrases as $keyphrase) {
					...
					$keyphrase_statements[] = " sh.keyphrase = '" . $keyphrase . "' ";	
				}
			}
...
*/
	...
	public function delete_search($id_list = array()) {
		...
		// If $id_list is empty the whole table will be cleared
