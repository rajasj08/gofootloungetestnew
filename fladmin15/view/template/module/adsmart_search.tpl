...
$device_xxs =  480;    // xxs : <   480 // doesn't exist on bootstrap 
...
<!-- Disabled
<script src="view/javascript/adsmartsearch_repository/jscroll.js"></script>
<link type="text/css" href="view/stylesheet/jscroll.css" rel="stylesheet" media="screen" />
-->
...
// Oc earlier than 1.5.2 uses jQuery 1.6.1 which doesn't implement functions like .on()
if (version_compare(VERSION, '1.5.2', '<') == true) { 

<!-- 	<script type="text/javascript" src="view/javascript/adSmartSrc_repository/jquery-1.7.1.min.js"> -->
...
jQuery.fn.on = function(eventsmap, selector, data, handler) {
...
// Update search cache
	function update_search_cache() {
		$.ajax({
			url: '<?php echo $catalog_url ?>index.php?route=module/adsmart_search/update_search_cache&token=<?php echo $token; ?>',
			dataType: 'html',
			beforeSend: function() {
...
// *******************
	...
	$("#max_num_results_slider").slider({                   
		...
		slide: function(event, ui) {
			...
			for (var i =  max_num_results+1; i <= num_results; i++ ){
				$('.adsmart_search li:nth-child('+i+')').css('display','none');
			}
			update_viewport();
		}
	});
	...
	// Init the slider cursor positions according to their value
...
// End Color Picker functions	
	...
	$('select[name="adsmart_search_dropdown_preset_style"]').change(function(){
...
// *******************
	...
	<?php require_once(DIR_CATALOG.'view/javascript/adsmartsearch_repository/adsmartsearch_livesrc_js.php') ?>
...
// Init Search Options window
	...
	<?php if (!isset($adsmart_search_first_boot )) { ?>
...
<?php
/* ****************************************
// Search input & Live Search style 
// ***************************************/
?>
<?php require_once(DIR_CATALOG.'view/theme/lexus_nextstore/stylesheet/adsmartsearch_livesrc_css.php') ?>
		...
		<div id="header-wrapper">
			...
			<div id="sticky-header">
				...
				<div class="buttons">
					...
					<a class="button orange" target="_blank" style="margin-right:10px;" href="<?php echo $catalog_url ?>index.php">Demo Store Front</a>
					<?php } ?>
							...
							<tr>
								...
								<td>
									<table style="width:100%">
										<thead>
											<tr>
												<th><?php echo $text_mysql_var_name; ?></th>
