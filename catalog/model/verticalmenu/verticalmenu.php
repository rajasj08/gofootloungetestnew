...
class ModelVerticalmenuVerticalmenu extends Model {		
	...
	public function genMegaMenuByConfig( $parentId, $level,$menu  ){
		...
		$class  = 'dropdown-menu';
		$output .= '<div class="'.$class.'" '.$attrw.' ><div class="dropdown-menu-inner test2">';
		...
		foreach( $menu['megaconfig']->rows  as $row ){
			...
			$output .= '<div class="row">';
				foreach( $row->cols as $col ){
					...
					$output .= '<div class="mega-col col-md-12 '.$colclass.'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
		...
		if( $parent['type_submenu'] == 'html' ){
			...
			if( $parent['colums'] > 1  ){
				...
				if( !empty($parent['megaconfig']->rows) ) {
					...
					$output = '<div class="'.$class.' level'.$level.'" '.$attrw.' ><div class="dropdown-menu-inner test3">';
					...
					$output = '<div class="'.$class.' mega-cols cols'.$parent['colums'].'" '.$attrw.' ><div class="dropdown-menu-inner test4"><div class="row">';
					$cols   = array_chunk( $data, ceil(count($data)/$parent['colums'])  );
			...
			///	echo '<pre>' .print_r( $parent, 1 );
				if( !empty($parent['megaconfig']->rows) ) {
					$output = '<div class="'.$class.' level'.$level.'" '.$attrw.' ><div class="dropdown-menu-inner test">';
					foreach( $parent['megaconfig']->rows as $rows ){ 
						foreach( $rows as $rowcols ){
							$output .='<div class="row">';
					...
					$output = '<div class="'.$class.' level'.$level.'" '.$attrw.' ><div class="dropdown-menu-inner test1">';
