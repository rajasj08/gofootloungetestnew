...
class ModelCatalogProductAdsmartSearch extends Model {
...
# INSERT INTO debug1 (db_field, partial_length,field_length, current_word, word, current_word_len, word_len, word_tolerance) VALUES (db_field, partial_length,field_length, current_word, word, current_word_len, word_len, word_tolerance);
	...
	public function adsmart_search_getProducts($data = array()){
		...
		// Since the array $current_search_options must be identical for a given result set (we make an MD5 from that array), 
		// we cannot have two different values for $data['filter_description']) ('' and false), even if their meaning is the same.
 		// Otherwise when caching is enabled, two identical cache files (and different MD5) will be created.
		...
		// Flag that tells if misspellings are enabled (exact matches can't include misspellings)
		...
		// better to apply these functions just one time than doing it every time we need the filter name
		if (!empty($data['filter_name'])) {
			...
			$search_string_to_unique_words	= array_unique($search_string_to_words);
			...
			if ($this->config->get('adsmart_search_include_plurals')){
				...
				$inflect = new inflect();
				foreach ($search_string_to_unique_words as $word) {
			...
			foreach ($search_string_to_unique_words as $word) {
				...
				$fulltext_exact_match_boolmode .= '+('.$word;
			...
			$words = array_unique(array_merge($search_string_to_unique_words, $inflection));
		...
		$is_live_search = (isset($data['live_search']) && $data['live_search'] == true) ? true : false;
																													...
																													if (ADSMART_SRC_DEBUG){
																														...
																														$debug_output .= '<br><br>Search string words - unique (array): '; if (isset($search_string_to_unique_words)) foreach ($search_string_to_unique_words as $word) { $debug_output .=  ' - '.$word; }
...
if(!empty($data['filter_name'])){
	...
	$LikeSearch=$data['filter_name'];

		$query = $this->db->query('SELECT oc_product.product_id,oc_product.model, oc_product.image, oc_product.price, oc_product_special.price AS special, oc_product_description.name
FROM `oc_product` , `oc_product_special` , `oc_product_description`
WHERE  `name`LIKE "'.$LikeSearch.'%" 
AND oc_product.product_id = oc_product_special.product_id
AND oc_product.product_id = oc_product_description.product_id 
...
$pid=$query->rows[$i]['product_id'];

$product_dataModel[$pid]=array('product_id'=>$query->rows[$i]['product_id'],'name'=>$query->rows[$i]['name'],'image'=>$query->rows[$i]['image'],'model'=>$query->rows[$i]['model'],'price'=>$query->rows[$i]['price'],'special'=>$query->rows[$i]['special']);
...
$product_dataModels[$pid]=array('product_id'=>$query->rows[$i]['product_id'],'name'=>$query->rows[$i]['name'],'image'=>$query->rows[$i]['image'],'model'=>$query->rows[$i]['model'],'price'=>$query->rows[$i]['price'],'special'=>$query->rows[$i]['special']);
...
if(count($product_dataModels)==0&&count($product_dataModel)==0){
...
$LikeSearch=$data['filter_name'];
		...
		$query = $this->db->query('SELECT oc_product.product_id,oc_product.model, oc_product.image, oc_product.price, oc_product_special.price AS special, oc_product_description.name
FROM `oc_product` , `oc_product_special` , `oc_product_description`
WHERE  `model`LIKE "'.$LikeSearch.'%" 
AND oc_product.product_id = oc_product_special.product_id
AND oc_product.product_id = oc_product_description.product_id
AND oc_product.status = 1
 ORDER BY product_id ASC LIMIT 10');

$product_dataModelmodel=[];
...
$product_dataModelmodel[$pid]=array('product_id'=>$query->rows[$i]['product_id'],'name'=>$query->rows[$i]['name'],'image'=>$query->rows[$i]['image'],'model'=>$query->rows[$i]['model'],'price'=>$query->rows[$i]['price'],'special'=>$query->rows[$i]['special']);



}

return  $product_dataModelmodel;
						...
						if ( $include_misspellings ){
							...
							foreach ($search_string_to_unique_words as $word) {	
								...
								if ($tolerance[$word] > 0) {
									...
									// This query might throw a division by zero error:
...
// Note: the match statement
//		is case insensitive.

						// The MATCH() AGAINST() operator doesn't allow wildcards to be 
						// preappended to strings like *word because the match starts from
						// the 1st character of an index. So we can't use MATCH to search
						// for partial words, we can only search for words ending with
						// some string. For exampe, the word 'wild*' will also match 'wildcards'
						// but '*wild*' cannot match 'bewilder'.
						// In this case we can only use the statement LIKE %word% that cannot take
						// advantage of FULL TEXT indexing but allows partial word matches.	
						// The MATCH() AGAINST() operator will be used to find words "starting with".
						...
						if ($this->config->get('adsmart_search_algorithm') == 'default'){
							...
							foreach ($words as $word) {
								...
								$sql_match ="";
								// Process the current word only if: 
								//	( FULL TEXT indexes enabled
								//		AND ( 
								//		     ( word length <  FULL TEXT minimum word length )   OR
								//		     ( word length >= FULL TEXT minimum word length    AND    word length >=  minimum length allowed for a partial word )
								//       )
								// OR 
								// FULL TEXT indexes not enabled )
								
								$include_partial_words = $word_length[$word] >= $this->config->get('adsmart_search_partial_word_length') && $this->config->get('adsmart_search_include_partial_words') == 1;
								
								
								if ( $include_partial_words OR $word == $last_search_string_word ) {
									$partial_word_operator = "%";
								}
								else {
									$partial_word_operator = "";
								}
								...
								if ( 
									( $this->config->get('adsmart_search_index_db') 
										&& (
											$word_length[$word] < $full_text_min_word_len 
									...
									$sql_match = " LCASE(".$db_field.") LIKE '". $partial_word_operator . $word . $partial_word_operator . "' ";
									$like_detected = true;
								}
						...
						if ( $this->config->get('adsmart_search_index_db') ) {
							...
							$sql_match = " MATCH (".$db_field.") AGAINST ('(".$fulltext_exact_match_boolmode.") ".$weighted_words."' IN BOOLEAN MODE ) ";
							//	$sql_match = " REGEXP '[[:<:]]".$word."[[:>:]]' "; // [[:>:]] are the word-boundary markers
...
// Include misspellings
						if ( $include_misspellings ){
							...
							foreach ($search_string_to_unique_words as $word) {	
								...
								if ($tolerance[$word] > 0) {
									$implode[] = "misspellings('".$word."', ".$db_field.", ".$tolerance[$word].") BETWEEN 0 AND ".$tolerance[$word];		
								}
							}
							if (count($implode) > 0){
		...
		// An handy way to know when we can paginate results by MySQL is to use the flag $we_can_paginate_by_mysql

		// Conditions that must be satisfied:
		// - the sort type is not by relevance		OR
		// - the sort type is by relevance AND there are not "LIKE" operators
		
		// Use the method registry->set() instead of a normal variable because this value will also be used outside of this method
		...
		if ( !empty($product_fields) && isset($data['sort']) && in_array($data['sort'], $sort_data) ) {
...
// This line is just for test purposes when the sorting by relevance is enabled, it speeds up the search:
		...
		$search_options = array('fn'	=> 'filter_name', 
								'ft'	=> 'filter_tag', 
								'ff'	=> 'filter_filter', 
...
// echo http_build_query($current_search_options); exit;
		...
		if ($this->config->get('adsmart_search_enable_search_cache') && $there_are_filters ){
			...
			$product_data = $this->cache->adsmart_search_get($hash, 'search_string');
			
			// together with $product_data, will also be available the total number of products,
		...
		if ( !count($product_data) ) { 
			...
			// Products are sorted by relevance by PHP only when we cannot make the pagination by MySQL
			if ( ! $we_can_paginate_by_mysql )  {
	...
	//										+		$relevance[$product_key] * $num_of_detected_words / $num_of_words_in_search_string / $num_words_in_value;
								...
								foreach ($product_value_array as $value){
									...
									foreach ($search_string_to_words as $word) {	
										...
										preg_match_all('/\b('.$word.')\b/', $value, $matches); 
										...
										if ($this->config->get('adsmart_search_include_plurals')){
											...
											if ( $inflection[$word] != $word ) {
												...
												preg_match_all('/\b('.$inflection[$word].')\b/', $value, $matches_inflections); 
			...
			// Move on top results with a lower levenshtein distance.
									if ( $include_misspellings ){
																													if (ADSMART_SRC_DEBUG){ $debug_output .=  '<br>*************** Misspellings ***************<br>';}	
										$distances = array();
										...
										foreach ($distances as $wrd => $dist) {
											...
											if ($dist > 0) {
												...
												$total_score += $distance_score;	
																												...
																												if (ADSMART_SRC_DEBUG){ $debug_output .=  '<br>Score for the Levenshtein distance calculated on the word: <b>'. $wrd.'</b>: '.$distance_score.'<br>';}											
										}	
									...
									// (see preg_match_all('/\b('.$word.')\b/', $value, $matches);  )
									$search_string_matches = array();

									if ( count($search_string_to_words) > 1 ){
										preg_match_all('/\b('.$data['filter_name'].')\b/', $value, $search_string_matches); 
									...
									$total_score += $exact_match_ratio * $num_exact_matches;
																													if (ADSMART_SRC_DEBUG){
																													$debug_output .=  '<br>Search string made of: '. count($search_string_to_words).' words<br>';
			...
			if ($this->config->get('adsmart_search_enable_search_cache') && $there_are_filters ){
				...
				// 2419200	= 1 month
				// 29030400	= 1 year			
				$this->cache->adsmart_search_set($current_search_options,  $product_data, $product_total, 'search_string', $this->config->get('adsmart_search_cache_update_frequency'));
			}
