...
class ModelAccountOrder extends Model {
	...
	public function validateorderno($order_id)//check order info
	{ 
		/* */
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_update` WHERE timestamp = '" . trim($order_id) . "'");

			
			if($query->num_rows > 0)
			{
			$n_orderid=$query->row['order_id'];

			$query1 = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$n_orderid . "' and customer_id='".(int)$this->session->data['customer_id']."'");
			
			return $query1->row['order_id'];
			}
		
		//} 
		return 0;
	}
        public function getsmsdetailByOrderId($orderid)//get sms details by order id
        { 
              if ($this->customer->isLogged())
		{
			$query1 = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ordersms` WHERE order_id = '" . (int)$orderid . "'");
			if ($query1->num_rows > 0) {
                            if( $query1->row['order_status_name']=='Completed' || $query1->row['order_status_name']=='Shipped' ) 
                        return $query1->rows;
                         }else { return 0;}
