...
class ModelPavdealsproduct extends Model {		
	...
	public function getProductSpecialsForDeals($data = array(), $flag = true) {
		...
		$sql = "SELECT DISTINCT ps.product_id,ps.date_start,ps.date_end,ps.price AS special,p.price AS price, (round(((p.price  - ps.price)/p.price)*100, 0)) As specialpercent,(SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) ".$join." LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' {$filter} GROUP BY ps.product_id";
		//return $sql;
		$sort_data = array(
			'pd.name',
			'p.date_added',
