...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$voucher_total = $this->model_account_order->getTotalOrderVouchersByOrderId($result['order_id']);
                     $smsdetails = $this->model_account_order->getsmsdetailByOrderId($result['order_id']);
	...
	public function info() { 
  		...
  		public function editorder()
  	{
		...
		$this->language->load('account/editorder');
		...
		$this->data['button_view'] = $this->language->get('button_view');
		$this->data['button_reorder'] = $this->language->get('button_reorder');
		$this->data['button_editorder'] = $this->language->get('button_editorder');
		$this->data['button_continue'] = $this->language->get('save'); 
		$this->data['button_back'] = $this->language->get('button_back');
		$this->data['entry_orderno']=$this->language->get('entry_orderno');
		$this->data['button_submitcontinue']=$this->language->get('button_submitcontinue'); 
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/editorder.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/editorder.tpl';
		} else {
			$this->template = 'default/template/account/editorder.tpl';
		}
  	...
  	public function ordersession()
  	{
  		...
  		$order_info = $this->model_account_order->validateorderno($orderno);
  		...
  		$this->session->data['order_id']=$order_info; 
  		echo 1;
  		}
  		else echo 0; 
  	}
  	
  	public function goorderinfos()//edit order  
  	{
		...
		$this->session->data['cart'] = array(); 
  		if (isset($this->session->data['order_id'])) {
			...
			$this->data['edit_orderid'] =$this->session->data['order_id']; 
			if ($order_info) {
				...
				foreach ($order_products as $order_product) {
					...
					$order_options = $this->model_account_order->getOrderOptions($this->session->data['order_id'], $order_product['order_product_id']);
  		...
  		public function unsetorder()//unset order
  	{
  		$this->load->model('account/order');
  		if (isset($this->session->data['order_id'])) {
			...
			if ($order_info) {
				...
				foreach ($order_products as $order_product) {
						...
						echo $this->cart->remove( $order_product['product_id']);	
